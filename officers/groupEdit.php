<?php
    ...
    if($_POST) {
        ...
        if(empty($validationMsg)) {
            $updateGroup = query('UPDATE groups SET name = :name WHERE id = :id', 
                                    array(
                                        'id' => $submittedData['id'],
                                        'name' => $submittedData['group_name']
                                ));
            if($updateGroup) { 
                header('Location: groupEdit.php?id='.$submittedData['id'].'&update_success');
            }            
        }
    ...
    <main id="main-content">
        ...
        <?php 
            ...
            if(empty($validationMsg['id']) && isset($_GET['id'])) {
                $queryOne = query('SELECT * FROM groups WHERE id = :id', array('id'=>$id));

                // if found data
                if($queryOne) {

        ?>  
                    <h1><a href="groupList.php">Group List</a> / Group Edit</h1>
                    <hr>

                    <form data-ready="false" method="post" action="groupEdit.php?id=<?= $id ?>">
                        ...
                        <label> Group Name:<br> 
                            <input required type="text" name="group_name" placeholder="Please input new group" value="<?= $queryOne[0]->name ?>">
                        </label>
                        <br><br>
