<?php
    ...
    if($_POST) {
        ...
        if(empty($validationMsg)) {
            ...
            $categoryData = array('name' => $submittedData['category_name']);

            // get parent key 
            $categoryField   = implode(",", array_keys($categoryData));
            $categoryValue   = ":".implode(",:", array_keys($categoryData));

            $categoryInsert  = query('INSERT INTO categories('.$categoryField.') VALUES ('.$categoryValue.')', $categoryData);

            if($categoryInsert) {
                header('location: categoryAdd.php?success');
            }
        }
    }
 ...
 ?>
    ...
    <main id="main-content">
        ...
        <h1><a href="categoryList.php">Category List</a> / Category Add </h1>
        <hr>

        <form method="post" action="categoryAdd.php">
            <p><b>Note:</b> Only accept letters and numbers</p>

            <input required type="text" name="category_name" placeholder="Please input new category" value="<?php output(@$category_name) ?>"><br><br>
            <?php 
