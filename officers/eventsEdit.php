<?php
    ...
    if($_POST) {
        ...
        if(empty($validationMsg)) {
            $updateCategory = query('UPDATE events SET event_name = :event_name WHERE id = :id',$submittedData);
            if($updateCategory) {
                header('Location: eventsEdit.php?id='.$submittedData['id'].'&update_success');
            }
        }
       ...
       <?php
            ...
            if($_GET) {
                ...
                if(isset($validationMsg['id'])) {
                    ...
                    echo '<h3>We cannot process your request please click <a href="eventsList.php">here</a> to go back to previous menu</h3>';
                }
        ...
        <?php
            ...
            if(empty($validationMsg['id']) && isset($_GET['id'])) {
                $queryOne = query('SELECT * FROM events WHERE id = :id', array('id'=>$id));

                // if found data
                if($queryOne) {

        ?>
                    <h1><a href="eventsList.php">Events List</a> / Event Edit</h1>
                    <hr>

                    <form data-ready="false" method="post" action="eventsEdit.php?id=<?= $id ?>">
                        ...
                        <label> Group Name:<br>
                            <input required type="text" name="event_name" placeholder="Please input new event name" value="<?= $queryOne[0]->event_name ?>">
                        </label>
                        <br><br>
