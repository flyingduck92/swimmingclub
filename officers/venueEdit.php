<?php
    ...
    if($_POST) {
        ...
        if(empty($validationMsg)) {
            $updateGroup = query('UPDATE venue SET name = :name WHERE id = :id',
                                    array(
                                        'id' => $submittedData['id'],
                                        'name' => $submittedData['venue_name']
                                ));
            if($updateGroup) {
                header('Location: venueEdit.php?id='.$submittedData['id'].'&update_success');
            }
        }
       ...
       <?php
        ...
        <?php
            ...
            if(empty($validationMsg['id']) && isset($_GET['id'])) {
                $queryOne = query('SELECT * FROM venue WHERE id = :id', array('id'=>$id));

                // if found data
                if($queryOne) {

        ?>
                    <h1><a href="venueList.php">Venue List</a> / Venue Edit</h1>
                    <hr>

                    <form data-ready="false" method="post" action="venueEdit.php?id=<?= $id ?>">
                        ...
                        <label> Group Name:<br>
                            <input required type="text" name="venue_name" placeholder="Please type new input here" value="<?= $queryOne[0]->name ?>">
                        </label>
                        <br><br>
